...
class Spider(object):
    ...
    def download_card(self, name='full', start=1001, end=1200, star=3):
        base = f'https://redive.estertion.win/card/{name}/'
        save_dir = os.path.join(os.path.dirname(__file__), f'card-{name}/')
        os.makedirs(save_dir, exist_ok=True)
    ...
    def download_card_profile(self, start=1001, end=1200, star=3):
        self.download_card('profile', start, end, 1)

    def download_card_actual_profile(self, start=1001, end=1200, star=3):
        self.download_card('actual_profile', start, end, 3)
...
if __name__ == '__main__':
    spider = Spider()
    # 运行过程出现404和200属正常情况
    #spider.download_icon_unit(start=1067, end=1067, star=3)
    #spider.download_icon_unit(start=1701, end=1703, star=1)
    #spider.download_icon_unit(start=1009, end=1023, star=6)
    #spider.download_icon_unit(start=1800, end=1802, star=3)
    #spider.download_icon_unit(start=1117, end=1119, star=3)
    #spider.download_icon_unit(start=1800, end=1802, star=1)
    # 如果需要爬取漫画，取消掉下面的这行注释
