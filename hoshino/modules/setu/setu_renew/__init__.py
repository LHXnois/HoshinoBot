...
'''
sv = hoshino.Service('setu-mix', bundle='setu', help_=HELP_MSG)
...
async def send_setu(bot, ev):
    ...
    if not is_su:
        ...
        if len(args) >= 2 and args[1].isdigit():
            gid = int(args[1])
        withdraw_status = "不撤回" if get_group_config(
            gid, "withdraw") == 0 else f'{get_group_config(gid, "withdraw")}秒'
        lolicon_status = "启用" if get_group_config(gid, "lolicon") else "禁用"
        lolicon_r18_status = "启用" if get_group_config(
            gid, "lolicon_r18") else "禁用"
