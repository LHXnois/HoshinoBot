    ...
    if not ret:
        ...
        while pls.get_on_off_status(gid):
            if datetime.now() < pls.timeout[gid] and ct<10:
                ...
                await bot.send(ev, f"由于超时，已为您自动退出搜图模式，以后要记得说“谢谢{NNICKNAME[0]}”来退出搜图模式噢~")
