...
async def get_pic(address):
    return await (await aiorequests.get(address, timeout=20)).content
...
class SauceNAO():
    ...
    async def get_sauce(self, url):
        ...
        response = await aiorequests.get(
            f'{self.host}/search.php', params=self.params, timeout=15)
        ...
        for sauce in sauces['results']:
            try:
                ...
                if not similarity.replace(".", "").isdigit():
                    # print(sauce)
                    similarity = 0
                simimax = float(similarity) if float(
                    similarity) > simimax else simimax
                thumbnail_url = sauce['header']['thumbnail']
                if THUMB_ON:
                    try:
                        thumbnail_image = str(MessageSegment.image(
                            pic2b64(ats_pic(Image.open(BytesIO(
                                await get_pic(thumbnail_url)))))))
                    except Exception as e:
                        print(e)
...
class ascii2d():
    ...
    async def get_search_data(self, url: str, data=None):
        if data is not None:
            ...
            html_data = await aiorequests.get(url, timeout=15)
        ...
        for data in all_data[1:self.num+1]:
            try:
                ...
                if not data.xpath('.//div[@class="detail-box gray-link"]/h6'):
                    ...
                    info_url = data2.xpath(
                        './/a/@href')[0].strip() if data.xpath('.//a/@href') else "no link"
                    tag = "外部登录" if info_url == "no link" else info_url.split(
                        '/')[2]
                else:
                    data2 = data.xpath(
                        './/div[@class="detail-box gray-link"]/h6')[0]
                    info_url = data2.xpath(".//a/@href")[0].strip()
                    tag = (data2.xpath("./small/text()")
                           or data2.xpath(".//a/text()"))[0].strip()
    ...
    async def get_view(self, ascii2d) -> str:
        ...
        try:
            html_index_data = await aiorequests.get(url_index, timeout=7)
